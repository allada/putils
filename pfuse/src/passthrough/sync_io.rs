...
use log::info;
use fuse_backend_rs::abi::fuse_abi::{CreateIn, FOPEN_IN_KILL_SUIDGID, WRITE_KILL_PRIV};
...
use fuse_backend_rs::bytes_to_cstr;
#[cfg(any(feature = "vhost-user-fs", feature = "virtiofs"))]
...
impl<S: BitmapSlice + Send + Sync> FileSystem for PassthroughFs<S> {
    ...
    fn init(&self, capable: FsOptions) -> io::Result<FsOptions> {
        ...
        if !self.cfg.do_import && capable.contains(FsOptions::ZERO_MESSAGE_OPEN) {
            opts |= FsOptions::ZERO_MESSAGE_OPEN;
            // We can't support FUSE_ATOMIC_O_TRUNC with do_import
            opts.remove(FsOptions::ATOMIC_O_TRUNC);
        }
        if !self.cfg.do_import && capable.contains(FsOptions::ZERO_MESSAGE_OPENDIR) {
