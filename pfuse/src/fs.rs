...
pub struct FileDesc {
    raw_fd: RawFd,
    // orig_ino: RefCell<Option<libc::ino_t>>,
}
...
impl FileDesc {
    pub fn open(path: impl AsRef<OsStr>, mut flags: libc::c_int) -> io::Result<Self> {
        ...
        let raw_fd = syscall!(open(c_path.as_ptr(), flags))?;
        Ok(Self{
            raw_fd,
            // orig_ino: RefCell::default(),
        })
    ...
    // pub fn ino(&self) -> io::Result<libc::ino_t> {
    //     match *self.orig_ino.borrow() {
    //         Some(ino) => Ok(ino),
    //         None => {
    //             let mut stat = mem::MaybeUninit::uninit();
    //             syscall!(fstat(self.as_raw_fd(), stat.as_mut_ptr()))?;
    //             let stats = unsafe { stat.assume_init() };
    //             self.orig_ino.replace(Some(stats.st_ino));
    //             Ok(stats.st_ino)
    //         }
    //     }
    // }
    ...
    pub fn openat(&self, path: impl AsRef<OsStr>, mut flags: libc::c_int) -> io::Result<Self> {
        ...
        let c_path = CString::new(path.as_bytes())?;
        let raw_fd = syscall!(openat(fd, c_path.as_ptr(), flags))?;
        Ok(Self{
            raw_fd,
            // orig_ino: RefCell::default(),
        })
    ...
    pub fn read_dir(&self) -> io::Result<ReadDir> {
        ...
        // TODO: asyncify.
        let dp = NonNull::new(unsafe { libc::fdopendir(fd.raw_fd) }) //
